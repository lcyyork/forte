...
void FCI_MO::compute_permanent_dipole() {
    ...
    for (int A = 0; A < nroot_; ++A) {
        ...
        std::vector<double> opdm_a(nactv_ * nactv_, 0.0);
        std::vector<double> opdm_b(nactv_ * nactv_, 0.0);
        ci_rdms.compute_1rdm(opdm_a, opdm_b);
...
void FCI_MO::compute_sa_ref(const int& level) {
    ...
    for (int n = 0, nentry = sa_info_.size(); n < nentry; ++n) {
        ...
        for (int i = 0; i < nroots; ++i) {
            ...
            //            auto opdm_a = std::make_shared<std::vector<double>>();
            //            auto opdm_b = std::make_shared<std::vector<double>>();

            //            compute_rdm(ci_rdms, 1, irrep, multi, i, i, {opdm_a, opdm_b}, do_disk);

            //            scale_add(L1a.data(), *opdm_a, weight);
            //            scale_add(L1b.data(), *opdm_b, weight);
            ...
            if (level >= 2) {
                ...
                //                auto tpdm_aa = std::make_shared<std::vector<double>>();
                //                auto tpdm_ab = std::make_shared<std::vector<double>>();
                //                auto tpdm_bb = std::make_shared<std::vector<double>>();

                //                compute_rdm(ci_rdms, 2, irrep, multi, i, i, {tpdm_aa, tpdm_ab,
                //                tpdm_bb}, do_disk);

                //                scale_add(L2aa.data(), *tpdm_aa, weight);
                //                scale_add(L2ab.data(), *tpdm_ab, weight);
                //                scale_add(L2bb.data(), *tpdm_bb, weight);
            }
            ...
            if (level >= 3 && threepdc == "MK") {
                ...
                //                auto tpdm_aaa = std::make_shared<std::vector<double>>();
                //                auto tpdm_aab = std::make_shared<std::vector<double>>();
                //                auto tpdm_abb = std::make_shared<std::vector<double>>();
                //                auto tpdm_bbb = std::make_shared<std::vector<double>>();

                //                compute_rdm(ci_rdms, 3, irrep, multi, i, i,
                //                            {tpdm_aaa, tpdm_aab, tpdm_abb, tpdm_bbb}, do_disk);

                //                scale_add(L3aaa.data(), *tpdm_aaa, weight);
                //                scale_add(L3aab.data(), *tpdm_aab, weight);
                //                scale_add(L3abb.data(), *tpdm_abb, weight);
                //                scale_add(L3bbb.data(), *tpdm_bbb, weight);
            }
        }
    } // end looping over all averaged states
...
Reference FCI_MO::transition_reference(int root1, int root2, bool multi_state, int entry,
    ...
    //    ambit::Tensor D1a, D1b, D2aa, D2ab, D2bb, D3aaa, D3aab, D3abb, D3bbb;
    ...
    //        D1a = ambit::Tensor::build(ambit::CoreTensor, "D1a", {nactv_, nactv_});
    //        D1b = ambit::Tensor::build(ambit::CoreTensor, "D1b", {nactv_, nactv_});

    //        auto filenames = density_filenames_generator(1, irrep, multi, root1, root2);
    //        bool files_exist = check_density_files(1, irrep, multi, root1, root2);

    //        if (safe_to_read_density_files_ && files_exist) {
    //            read_disk_vector_double(filenames[0], D1a.data());
    //            read_disk_vector_double(filenames[1], D1b.data());
    //        } else {
    //            ci_rdms.compute_1rdm(D1a.data(), D1b.data());

    //            if (files_exist) {
    //                remove_density_files(1, irrep, multi, root1, root2);
    //            }

    //            if (disk) {
    //                write_disk_vector_double(filenames[0], D1a.data());
    //                write_disk_vector_double(filenames[1], D1b.data());
    //            }
    //        }
    ...
    //        //        ci_rdms.compute_1rdm(opdm_a, opdm_b);
    //        //        ref.set_G1(opdm_a, opdm_b, nactv_);
    ...
    //        D2aa = ambit::Tensor::build(ambit::CoreTensor, "D2aa", {nactv_, nactv_, nactv_,
    //        nactv_});
    //        D2ab = ambit::Tensor::build(ambit::CoreTensor, "D2ab", {nactv_, nactv_, nactv_,
    //        nactv_});
    //        D2bb = ambit::Tensor::build(ambit::CoreTensor, "D2bb", {nactv_, nactv_, nactv_,
    //        nactv_});

    //        auto filenames = density_filenames_generator(2, irrep, multi, root1, root2);
    //        bool files_exist = check_density_files(2, irrep, multi, root1, root2);

    //        if (safe_to_read_density_files_ && files_exist) {
    //            read_disk_vector_double(filenames[0], D2aa.data());
    //            read_disk_vector_double(filenames[1], D2ab.data());
    //            read_disk_vector_double(filenames[2], D2bb.data());
    //        } else {
    //            ci_rdms.compute_2rdm(D2aa.data(), D2ab.data(), D2bb.data());

    //            if (files_exist) {
    //                remove_density_files(2, irrep, multi, root1, root2);
    //            }

    //            if (disk) {
    //                write_disk_vector_double(filenames[0], D2aa.data());
    //                write_disk_vector_double(filenames[1], D2ab.data());
    //                write_disk_vector_double(filenames[2], D2bb.data());
    //            }
    //        }

    //        if (do_cumulant) {
    //            add_wedge_cu2(D1a, D1b, D2aa, D2ab, D2bb);
    //            ref.set_L2(D2aa, D2ab, D2bb);
    //        } else {
    //            ref.set_G2(D2aa, D2ab, D2bb);
    //        }

    //        //        ci_rdms.compute_2rdm(tpdm_aa, tpdm_ab, tpdm_bb);
    //        //        ref.set_G2(tpdm_aa, tpdm_ab, tpdm_bb, nactv_, do_cumulant);
    ...
    //        D3aaa = ambit::Tensor::build(ambit::CoreTensor, "D3aaa", std::vector<size_t>(6,
    //        nactv_));
    //        D3aab = ambit::Tensor::build(ambit::CoreTensor, "D3aab", std::vector<size_t>(6,
    //        nactv_));
    //        D3abb = ambit::Tensor::build(ambit::CoreTensor, "D3abb", std::vector<size_t>(6,
    //        nactv_));
    //        D3bbb = ambit::Tensor::build(ambit::CoreTensor, "D3bbb", std::vector<size_t>(6,
    //        nactv_));

    //        auto filenames = density_filenames_generator(3, irrep, multi, root1, root2);
    //        bool files_exist = check_density_files(3, irrep, multi, root1, root2);

    //        if (safe_to_read_density_files_ && files_exist) {
    //            read_disk_vector_double(filenames[0], D3aaa.data());
    //            read_disk_vector_double(filenames[1], D3aab.data());
    //            read_disk_vector_double(filenames[2], D3abb.data());
    //            read_disk_vector_double(filenames[3], D3bbb.data());
    //        } else {
    //            ci_rdms.compute_3rdm(D3aaa.data(), D3aab.data(), D3abb.data(), D3bbb.data());

    //            if (files_exist) {
    //                remove_density_files(3, irrep, multi, root1, root2);
    //            }

    //            if (disk) {
    //                write_disk_vector_double(filenames[0], D3aaa.data());
    //                write_disk_vector_double(filenames[1], D3aab.data());
    //                write_disk_vector_double(filenames[2], D3abb.data());
    //                write_disk_vector_double(filenames[3], D3bbb.data());
    //            }
    //        }

    //        if (do_cumulant) {
    //            add_wedge_cu3(D1a, D1b, D2aa, D2ab, D2bb, D3aaa, D3aab, D3abb, D3bbb);
    //            ref.set_L3(D3aaa, D3aab, D3abb, D3bbb);
    //        } else {
    //            ref.set_G3(D3aaa, D3aab, D3abb, D3bbb);
    //        }

    //        //        ci_rdms.compute_3rdm(tpdm_aaa, tpdm_aab, tpdm_abb, tpdm_bbb);
    //        //        ref.set_G3(tpdm_aaa, tpdm_aab, tpdm_abb, tpdm_bbb, nactv_, do_cumulant);
