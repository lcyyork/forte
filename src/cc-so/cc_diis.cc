...
void CC_SO::diis_manager_init() {
    diis_manager_ = std::make_shared<DIISManager>(diis_max_vec_, "CC_SO DIIS",
    ...
    if (not do_triples_) {
        size_t t1_nel = T1_.block("cv").numel();
        size_t t2_nel = T2_.block("ccvv").numel();
        ...
        size_t t1_nel = T1_.block("cv").numel();
        size_t t2_nel = T2_.block("ccvv").numel();
        size_t t3_nel = T3_.block("cccvvv").numel();
...
void CC_SO::diis_manager_add_entry() {
    if (not do_triples_) {
...
void CC_SO::diis_manager_extrapolate() {
    if (not do_triples_) {
...
void CC_SO::diis_manager_cleanup() {
    amp_ptrs_.clear();
    res_ptrs_.clear();
}
} // namespace forte
