...
std::unique_ptr<CC_SA> make_cc_sa(RDMs rdms, std::shared_ptr<SCFInfo> scf_info,
                                  std::shared_ptr<ForteOptions> options,
                                  std::shared_ptr<ForteIntegrals> ints,
                                  std::shared_ptr<MOSpaceInfo> mo_space_info) {
    return std::make_unique<CC_SA>(rdms, scf_info, options, ints, mo_space_info);
}

CC_SA::CC_SA(RDMs rdms, std::shared_ptr<SCFInfo> scf_info,
    ...
    print_method_banner(
    {"Spin-Adapted Coupled Cluster Using Generated Equations", "Chenyang Li"});
    startup();
    print_summary();
}
...
std::shared_ptr<ActiveSpaceIntegrals> CC_SA::compute_Heff_actv() {
...
void CC_SA::startup() {
    ...
    BTF_->add_mo_space("c", "i,j,k,l,m,n,c0,c1,c2,c3,c4,c5,c6,c7,c8,c9", core_mos_, NoSpin);
    BTF_->add_mo_space("v", "a,b,c,d,e,f,v0,v1,v2,v3,v4,v5,v6,v7,v8,v9", virt_mos_, NoSpin);
    ...
    F_["pq"] += 2.0 * V_["pnqm"] * K1["mn"];
    F_["pq"] -= V_["pnmq"] * K1["mn"];

    // obtain diagonal elements of Fock matrix
    Fd_ = std::vector<double>(nmo_);
    F_.citerate([&](const std::vector<size_t>& i, const std::vector<SpinType>&, const double& value) {
        if (i[0] == i[1]) {
            Fd_[i[0]] = value;
        }
    });
    ...
    for (size_t i = 0; i < nmo_; ++i) {
        bool find_a = std::find(core_mos_.begin(), core_mos_.end(), i) != core_mos_.end();
        outfile->Printf("\n    %3zu %11.6f(%d) %11.6f(%d)", i + 1, Fd_[i], find_a, Fd_[i], find_a);
    }
    outfile->Printf("\n    ---------------------------------");
}

void CC_SA::print_summary() {
    // Print a summary
...
//        }
    ...
    if (not converged) {
        throw PSIEXCEPTION("CC_SA computation did not converged.");
    }
...
//void CC_SA::rotate_hamiltonian(double& Eeff, BlockedTensor& Fnew, BlockedTensor& Vnew) {
