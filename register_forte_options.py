...
def register_forte_options(options):
    ...
    register_cc_so_options(options)
    register_old_options(options)
    register_psi_options(options)

def register_driver_options(options):
    options.set_group("")
    options.add_str('JOB_TYPE', 'NEWDRIVER', ['NONE','NEWDRIVER','MR-DSRG-PT2', 'CASSCF'],
...
def register_dsrg_options(options):
    ...
    options.add_int("LDSRG3_NCOMM_3BODY", 0,
    ...
    options.add_str("LDSRG3_PERTURB_TYPE", "FOCK", ["FOCK", "FINK"],
                      "The perturbation type for truncating LDSRG3 terms")

    options.add_int("LDSRG3_FINK_ORDER", 8,
    ...
    options.add_int("LDSRG3_FOCK_ORDER", 8,
                    "The perturbation order of energy terms included in LDSRG3 single commutator"
    ...
    options.add_bool("LDSRG3_ZERO_T3", False, "Zero all T3 amplitudes for testing")

    options.add_bool("LDSRG3_DIAG_HBAR3_ONLY", False,
    ...
    options.add_bool("LDSRG2STAR_DO_DIAGONAL", True,
                     "Compute diagonal components of Hbar when doing LDSRG(2*) if True")

    options.add_bool("LDSRG2STAR_DO_HIGH_SCALING_TERMS", True,
