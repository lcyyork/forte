...
def register_driver_options(options):
    ...
    options.add_str("SCF_TYPE", None,
                    "The integrals used in the SCF calculation")
    options.add_str("REF_TYPE", 'SCF', ['SCF', 'CASSCF'],
    ...
    options.add_double("E_CONVERGENCE", 1.0e-9,
...
def register_aci_options(options):
    ...
    options.add_bool("SPIN_TEST", False,
...
def register_davidson_liu_options(options):
    ...
    options.add_int("DL_GUESS_SIZE", 50,
                    "Set the number of determinants in the initial guess"
...
def register_asci_options(options):
    ...
    options.add_double("ASCI_E_CONVERGENCE", 1e-5,
    ...
    options.add_double("ASCI_PRESCREEN_THRESHOLD", 1e-12,
                       "ASCI prescreening threshold")


def register_fci_mo_options(options):
    options.set_group("FCIMO")
    options.add_str("FCIMO_ACTV_TYPE", "COMPLETE",
    ...
    options.add_str("FCIMO_IPEA", "NONE", ["NONE", "IP", "EA"],
...
def register_dsrg_options(options):
    ...
    options.add_bool("DSRG_DIPOLE", False,
    ...
    options.add_int("MAXITER_RELAX_REF", 15,
    ...
    options.add_bool("DSRGPT", True,
                     "Renormalize (if true) the integrals for purturbitive"
    ...
    options.add_bool("DSRG_HBAR_SEQ", False,
    ...
    options.add_bool("PRINT_1BODY_EVALS", False,
                     "Print eigenvalues of 1-body effective H")

    options.add_bool("DSRG_MRPT3_BATCHED", False,
                     "Force running the DSRG-MRPT3 code using the batched algorithm")

    options.add_bool("IGNORE_MEMORY_WARNINGS", False,
                     "Force running the DSRG-MRPT3 code using the batched algorithm")

    options.add_int("DSRG_DIIS_START", 2,
                    "Iteration cycle to start adding error vectors for"
                    " DSRG DIIS (< 1 for not doing DIIS)")

    options.add_int("DSRG_DIIS_FREQ", 1,
...
def register_dwms_options(options):
    ...
    options.add_bool("DWMS_ITERATE", False,
                     "Iterative update the reference CI coefficients in"
...
def register_casscf_options(options):
    ...
    options.add_int("CASSCF_DIIS_MIN_VEC", 2,
    ...
    options.add_int("CASSCF_DIIS_START", 2,
...
def register_old_options(options):
    ...
    options.add_str("SRG_ODEINT", "FEHLBERG78",
