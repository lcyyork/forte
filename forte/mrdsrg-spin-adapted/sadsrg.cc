...
std::shared_ptr<ActiveSpaceIntegrals> SADSRG::compute_Heff_actv() {
    ...
    if (foptions_->get_bool("FORM_HBAR3")) {
        ...
        deGNO_ints("Hamiltonian", Edsrg, Hbar1_, Hbar2_, H1, H2);
        // HACK: only works when the original orbitals are semicanonicalized
...
void SADSRG::deGNO_ints(const std::string& name, double& H0, BlockedTensor& H1, BlockedTensor& H2,
    ...
    if (not complete_valence_ints_) {
        H2t("pqrs") = H2.block("aaaa")("pqrs");
        H1t("pq") = H1.block("aa")("pq");
        ...
        // scalar from H1
        double scalar1 = -H1t("vu") * L1("uv");

        // scalar from H2
        double scalar2 = 0.25 * L1("uv") * temp("vyux") * L1("xy");
        scalar2 -= 0.5 * H2t("xyuv") * L2("uvxy");
        ...
        std::vector<size_t> vdocc_in_rdocc =
            mo_space_info_->pos_in_space("VALENCE_DOCC", "RESTRICTED_DOCC");
        std::vector<size_t> vuocc_in_ruocc =
            mo_space_info_->pos_in_space("VALENCE_UOCC", "RESTRICTED_UOCC");
        ...
        std::vector<size_t> vdocc_in_valence =
            mo_space_info_->pos_in_space("VALENCE_DOCC", "POST_DSRG_ACTIVE");
        std::vector<size_t> vuocc_in_valence =
            mo_space_info_->pos_in_space("VALENCE_UOCC", "POST_DSRG_ACTIVE");
        std::vector<size_t> actv_in_valence;
        for (size_t i = 0; i < no; ++i) {
            auto in_vdocc = std::find(vdocc_in_valence.begin(), vdocc_in_valence.end(), i);
            auto in_vuocc = std::find(vuocc_in_valence.begin(), vuocc_in_valence.end(), i);
            if (in_vdocc == vdocc_in_valence.end() and in_vuocc == vuocc_in_valence.end())
                actv_in_valence.push_back(i);
        }
