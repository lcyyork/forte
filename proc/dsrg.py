...
class ProcedureDSRG:
    ...
    def compute_energy(self):
        ...
        for n in range(self.relax_maxiter):
            ...
            e_relax = self.solve_for_active_space()
            self.energies.append((e_dsrg, e_relax))
    ...
    def solve_for_active_space(self):
        """ Solve for active space and return the energy. """
        state_energies_list = self.active_space_solver.compute_energy()
        return forte.compute_average_state_energy(state_energies_list, self.state_weights_map)

    def solve_for_dsrg(self):
        """ Solve for DSRG and return the energy. """
        ...
        if self.do_brueckner:
            e_dsrg = self.compute_brueckner_orbitals()
    ...
    def test_relaxation_convergence(self, n):
        ...
        return self.relax_converged

    def compute_brueckner_orbitals(self):
        """ Compute Bruckner orbitals based on DSRG formalism. """
        e_dsrg = 0.0
        brueckner_converged = False
        n = self.brueckner_maxiter
        ...
        if not brueckner_converged:
            raise psi4.p4util.PsiException(f"DSRG Brueckner update did not converge in {n} cycles!")
    ...
    def print_summary(self):
        ...
        if self.do_dipole and (not self.do_multi_state):
            ...
            if self.relax_ref == "ITERATE" and self.relax_converged:
